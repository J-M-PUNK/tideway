            ...
            <div className="flex min-w-0 flex-1 items-center gap-3">
              ...
              */}
              <button
                key={`cover-${track.id}`}
                onClick={onExpand}
                className="group relative h-14 w-14 shrink-0 animate-track-change overflow-hidden rounded bg-secondary"
...
function EmptyPlayerBar({ onToggleQueue }: { onToggleQueue: () => void }) {
  return (
    <div className="border-t border-border bg-[hsl(var(--now-playing-bg))] px-4 py-3">
      <div className="flex items-center gap-4">
        <div className="flex min-w-0 flex-1 items-center gap-3">
          <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded bg-secondary text-muted-foreground">
            <Music className="h-5 w-5" />
          </div>
